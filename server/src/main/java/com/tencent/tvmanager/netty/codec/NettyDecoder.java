...
public class NettyDecoder extends ByteToMessageDecoder {
    ...
    public MessageLite decodeProtobufBody(byte msgType, byte busynissType, byte[] array) throws Exception {
        if (msgType == Header.MsgType.PAYLOAD) {
            ...
            if (busynissType == Header.BusinessType.REQUEST_OPEN_APP
                    || busynissType == Header.BusinessType.REQUEST_REMOVE_APP
                    || busynissType == Header.BusinessType.REQUEST_UPDATE_APP
                    || busynissType == Header.BusinessType.REQUEST_INSTALL_APP) {
                return AppActionRequestProto.AppActionRequest.getDefaultInstance()
                        .getParserForType().parseFrom(array);
            }
        }
